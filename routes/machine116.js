...
router.post("/machine116", (req, res) => {
  ...
  var flag_machine116 = processJSON("flag_machine116")
  
  console.log({QFG})

  flag_machine116 = flag_machine116 + 1
  ...
  if(machine116){
    ...
    var AXP = convertToDecimalDegrees(Number(req.body.auxiliaryAPD), Number(req.body.auxiliaryAPM))
    console.log({AXP})
    D0 = convertToDecimalDegrees(Number(req.body.newD0D), Number(req.body.newD0M))
    var AXG = convertToDecimalDegrees(Number(req.body.auxiliaryAGD), Number(req.body.auxiliaryAGM))
    IG = convertToDecimalDegrees(Number(req.body.newIGD), Number(req.body.newIGM))
  //2720 
    if(rightHand && flag_machine116 <= 1){
      console.log({D0})
      D0 = 360 - D0
      C0 = 270 - G8 + AXP + JP
      console.log({C0, G8, AXP, JP})
      console.log({G8, JP, AXP})
      var E0 = -90 - G9 + AXG - JG
      QFP = 90 - G8 - QFP 
      QFG = 90 - G9 + QFG
    ...
    if (leftHand){
      ...
      C0 = -90 - G8 + AXP - JP
      console.log({C0, G8, AXP, JP})
      var E0 = 270 - G9 + AXG + JG
      console.log({E0, G9, AXG, JG})
      QFP = 90 - G8 + QFP
      QFG = 90 - G9 - QFG
      console.log({QFG})
      //GOTO 2820
    ...
    res.render('homepage_continue', {AXP, AXG, D0, IG});
...
function convertToDecimalDegrees(D, M){
  return M/60 + D
}
