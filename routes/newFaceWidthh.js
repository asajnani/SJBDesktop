...
router.post("/newFaceWidthh", (req, res) => {

  var flag_newF = processJSON("flag_newF")

  flag_newF = flag_newF + 1

  const jsonString = fs.readFileSync("./routes/temp.json");
  const temp = JSON.parse(jsonString);

  temp.variables[0]["flag_newF"] = flag_newF
  

  fs.writeFileSync("./routes/temp.json", JSON.stringify(temp));


  res.render('homepage_newFaceWidth')
