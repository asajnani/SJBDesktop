...
router.post("/diff_sums", (req, res) => {
  ...
  var CFP = Number(req.body.pinionFinishingCutter)

  console.log("ACTUAL GEAR FINISHING CUTTER #?")

  var CFG = Number(req.body.gearFinishingCutter) 
  ...
  if(Math.abs(CF - F3) > 3){
    ...
    res.render('homepage_diffDiameters', {F3, CF} );
  }
  if (Math.abs(CF - F3) <= 3) {
    ...
    console.log({BCV, PI, SPV, K2})
    console.log({BCX})
    console.log("FNASN(SG/K2)===" + FNASN(SG / K2))
    var BGF = 2 * 180 / PI * FNASN(SG / K2)
    console.log({BGF, PI, SG, K2})
...
function FNASN(x){
  return Math.atan(x / Math.sqrt(((x * x) * 1) + 1))
}

function FNACS(x){
  return (-1 * (Math.atan(x / Math.sqrt(((x * x) * 1) + 1)))) + Math.PI / 2
}
...
function backlash_selection(index) {
